 ...
 *
 * @param mysqli  $connection The db connection object
...
if ( $step >= 3 ) {
	...
	$all_tables_mysqli = @mysqli_query($connection, 'SHOW TABLES' );

	if ( ! $all_tables_mysqli ) {
		$errors[] = mysqli_error($connection);
		$step = 2;
	} else {
		while ( $table = mysqli_fetch_array( $all_tables_mysqli ) ) {
			$all_tables[] = $table[ 0 ];
		}
	}
}
